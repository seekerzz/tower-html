...
# --- Existing Unit Logic (Refactored to check constraints if needed) ---
	...
	for dx in range(w):
		for dy in range(h):
			...
			if tiles.has(t_key):
				...
				# If unit acts as obstacle (most do?), register it
				# register_obstacle(Vector2i(x+dx, y+dy), unit) # We might want this?
				# The requirements mainly talk about "Obstacle System" separately.
				# But logically units should probably block path or increase weight?
				# The requirement says "register_obstacle... record obstacle and update pathfinding weight".
				# It doesn't explicitly say Units are obstacles for this task, but usually they are.
				# However, the task specifically mentions "Obstacle System" and "Initial Map" with static obstacles.
				# I will leave unit logic as is for now regarding AStar, unless I see a need.
				# Wait, "Obstacle System: implement register_obstacle... remove_obstacle".
				# I should probably expose these so other systems (like placement) can call them.
...
# Expansion Logic - Removed/Commented as we are moving to fixed grid, but keeping method sigs if referenced elsewhere.
# Actually, the task is "Refactor... to 19x11 grid".
# So previous expansion logic might be obsolete.
# I will keep the functions empty or adapt them if they are called by UI.
