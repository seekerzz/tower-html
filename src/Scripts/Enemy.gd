...
# Visual Component
var visual_controller: VisualController = null
...
func setup(key: String, wave: int):
	...
	if anim_config.has("style"):
		visual_controller.anim_style = anim_config.style
	if anim_config.has("amplitude"):
		visual_controller.idle_amplitude = anim_config.amplitude
	if anim_config.has("base_freq"):
		visual_controller.idle_freq = anim_config.base_freq
...
func _process(delta):
	...
	if visual_controller:
		...
		if target_node:
			visual_controller.apply_transform(target_node)

			# Apply flip based on movement direction
			# VisualController manages wobble_scale, which is always positive in X usually (or driven by sin)
			# We enforce flip direction here on top of the wobble
			if is_facing_left:
				if target_node.scale.x > 0:
					target_node.scale.x = -target_node.scale.x
			else:
				if target_node.scale.x < 0:
...
func _process_effects(delta):
	...
	# Flip logic moved to _process to avoid race condition with VisualController
...
func play_attack_animation(target_pos: Vector2, hit_callback: Callable = Callable()):
	...
	if anim_type == "elastic_shoot" and visual_controller:
		# Use component logic
		# Fire happens at 0.2s mark in animation usually.
		get_tree().create_timer(0.2).timeout.connect(func():
			if is_instance_valid(self) and hit_callback.is_valid():
				hit_callback.call()
		)
		visual_controller.play_elastic_shoot()

	elif anim_type == "elastic_slash" and visual_controller:
		visual_controller.play_elastic_slash()
		get_tree().create_timer(0.2).timeout.connect(func():
...
func die():
	...
	if enemy_data.get("is_boss", false) and visual_controller:
		# Disable physics/collision during death anim
		collision_layer = 0
		collision_mask = 0
		...
		await visual_controller.play_death_implosion(target_node)
