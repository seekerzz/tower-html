	...
	match stat_type:
		"attack":
			if "damage_mult" in host:
				...
				# Ensure we don't go below 0 (though unlikely with small steals)
				if host.damage_mult < 0: host.damage_mult = 0.0
		"attack_speed":
			if "attack_speed_mult" in host:
				host.attack_speed_mult -= amount
				applied_amount = amount
				if host.attack_speed_mult < 0.1: host.attack_speed_mult = 0.1 # Minimum cap
