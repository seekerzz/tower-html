...
func refresh_skills():
	...
	for i in range(units_with_skills.size()):
		...
		# Style
		var bg_color = UIConstants.COLORS.dark_bg
		var style = StyleMaker.get_flat_style(bg_color, UIConstants.CORNER_RADIUS.large, 2, UIConstants.COLORS.primary)
		...
		if style is StyleBoxFlat:
			...
			if GameManager.mana < unit.skill_mana_cost:
				if unit.skill_cooldown <= 0:
					...
					style.border_color = UIConstants.COLORS.danger
				...
				# Enough mana
				card.modulate = Color.WHITE
				style.border_color = UIConstants.COLORS.primary
