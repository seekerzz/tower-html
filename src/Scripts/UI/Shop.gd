...
func _apply_styles():
	...
	# Style Buttons
	apply_button_style(refresh_btn, UIConstants.COLORS.primary) # Blue
	apply_button_style(expand_btn, UIConstants.COLORS.success) # Green
	apply_button_style(start_wave_btn, UIConstants.COLORS.danger, true) # Red, Main Action

func apply_button_style(button: Button, color: Color, is_main_action: bool = false):
	var corner_radius = UIConstants.CORNER_RADIUS.large
