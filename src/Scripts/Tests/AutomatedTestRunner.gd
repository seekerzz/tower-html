...
func _setup_test():
	...
	if config.has("units"):
		for u in config["units"]:
			var ux = int(u.x)
			var uy = int(u.y)
			var key = GameManager.grid_manager.get_tile_key(ux, uy)
			...
			GameManager.grid_manager.place_unit(u.id, ux, uy)
...
func _spawn_random_trap(trap_id: String):
	# Map trap_id if needed
	if trap_id == "poison_trap": trap_id = "poison"
	elif trap_id == "fang_trap": trap_id = "fang"

	var valid_positions = []
	var gm = GameManager.grid_manager

	# Use active territory tiles to ensure we spawn on unlocked tiles
	for tile in gm.active_territory_tiles:
	...
	if valid_positions.size() > 0:
		...
		gm.spawn_trap_custom(pos, trap_id)
		print("[TestRunner] Spawned trap ", trap_id, " at ", pos)
	else:
		printerr("[TestRunner] No valid position for trap ", trap_id)
