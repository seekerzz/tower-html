...
func _ready():
	...
	if config.has("core_health"):
		...
		GameManager.max_core_health = config["core_health"]
...
func _setup_test():
	...
	if config.has("units"):
		for u in config["units"]:
			...
			if u.has("level") and u.level > 1:
				# key is already defined above in this scope
	...
	# Disable standard spawning if we are managing enemies manually
...
func _verify_damage_test():
	...
	if damaged:
		print("[TestRunner] VERIFY DAMAGE: PASSED - Damage events found.")
	else:
