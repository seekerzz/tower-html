...
const DEFAULT_BEHAVIOR_SCRIPT = preload("res://src/Scripts/Units/DefaultBehavior.gd")
...
func _get(property):
	if behavior:
		if property in behavior:
			return behavior.get(property)
	return null

func _set(property, value):
	if behavior:
		if property in behavior:
			behavior.set(property, value)
			return true
	return false
...
func setup(key: String):
	...
	# Load Behavior
	var behavior_script_path = "res://src/Scripts/Units/Behaviors/" + type_key.capitalize().replace(" ", "") + ".gd"
	# snake_case to PascalCase (simple approximation or use String methods if available, but capitalize helps)
	# "archer_rat" -> "ArcherRat". "viper" -> "Viper".
	var parts = type_key.split("_")
	var class_name_str = ""
	for part in parts:
		class_name_str += part.capitalize()

	behavior_script_path = "res://src/Scripts/Units/Behaviors/" + class_name_str + ".gd"

	if ResourceLoader.exists(behavior_script_path):
		var script = load(behavior_script_path)
		behavior = script.new(self)
	else:
		behavior = DEFAULT_BEHAVIOR_SCRIPT.new(self)
...
func attempt_placement(grid_pos: Vector2i):
	if behavior:
		behavior.on_placement_attempt(grid_pos)
...
func capture_bullet(bullet_snapshot: Dictionary):
	if behavior:
		behavior.on_bullet_captured(bullet_snapshot)

func get_trap_type() -> String:
	if behavior:
		return behavior.get_trap_type()
	return ""
