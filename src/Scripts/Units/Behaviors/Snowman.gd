...
func on_setup():
	_update_production_timer()

func _update_production_timer():
	if unit.unit_data.has("production_interval"):
		max_production_timer = unit.unit_data.production_interval
...
func on_tick(delta: float):
	...
	if production_timer <= 0:
		var item_id = unit.unit_data.get("produce_item_id", "snowball_trap")
