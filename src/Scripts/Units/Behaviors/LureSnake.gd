...
func on_setup():
	if unit.level >= 2:
		max_traps = 2
	_connect_to_existing_traps()

func _connect_to_existing_traps():
	if not GameManager.grid_manager: return
	if not GameManager.grid_manager.obstacles: return

	for pos in GameManager.grid_manager.obstacles:
		var trap = GameManager.grid_manager.obstacles[pos]
