...
func on_skill_activated():
	...
	# If range is 0 (global), maybe pick random valid tile?
	# For now, stick to original logic but ensure range is used correctly.
	# LureSnake has range 0 in game_data.json, but maybe it should have range if it places traps?
	# Or maybe it places traps on random enemies?
	# The test case uses "setup_actions": [{"type": "place_trap", ...}] which might bypass this.
	# However, if the unit uses skill, it needs a target.
