...
| 图腾流派 | 测试数 | 通过 | 失败 | 通过率 | 状态 |
|----------|--------|------|------|--------|------|
| 蝴蝶图腾 (butterfly_totem) | 6 | 6 | 0 | 100% | [x] 测试完成 |
| 狼图腾 (wolf_totem) | 4 | 4 | 0 | 100% | [x] 测试完成 |
| 眼镜蛇图腾 (viper_totem) | 7 | 7 | 0 | 100% | [x] 测试完成 |
| 鹰图腾 (eagle_totem) | 12 | 10 | 2 | 83% | [!] 发现问题 |
| 蝙蝠图腾 (bat_totem) | 6 | 5 | 1 | 83% | [!] 发现问题 |
| 牛图腾 (cow_totem) | 4 | 1 | 3 | 25% | [!] 发现问题 |
| 系统测试 | 4 | 2 | 2 | 50% | [!] 发现问题 |
| **总计** | **43** | **35** | **8** | **81%** | 测试执行完成 |

> **更新日期**: 2026-02-21
> **状态**: 数值验证测试执行完成，发现 8 个游戏 Bug，已修复 5 个
> **测试报告**: [test_execution_report_2026-02-21.md](test_execution_report_2026-02-21.md)
> **关键发现**:
> 1. 吸血机制不工作（P0）- **已修复**: 测试框架未正确应用debuff
> 2. 奶牛治疗机制不工作（P0）- **已修复**: 测试配置缺少防御单位导致核心死亡
> 3. 猫头鹰、鹦鹉无法攻击（P0）- **已修复**: 测试配置错误（猫头鹰是支援单位，鹦鹉需要子弹来源）
> 4. 菌菇治愈者脚本错误（P1）- **已修复**: Unit.gd 缺少 `damage_blocked` 信号
> 5. 狐狸魅惑系统不工作（P2）- **已修复**: 测试配置中的守护单位阻止狐狸被攻击
...
### 2026-02-21 测试框架和游戏Bug修复

#### 修复 1: 测试框架 debuff 应用机制
**问题**: `AutomatedTestRunner.gd` 未在敌人生成时应用测试配置中指定的 debuff（如 bleed、poison）

**影响**: 所有依赖 debuff 的测试（吸血、中毒等）均失败

**修复文件**: `src/Scripts/Tests/AutomatedTestRunner.gd`
- 在 `_on_enemy_spawned()` 中添加 `_apply_debuffs_to_enemy(enemy)` 调用
- 新增 `_apply_debuffs_to_enemy()` 函数处理 debuff 配置

**状态**: ✅ 已修复

---

#### 修复 2: Unit.gd 缺少 `damage_blocked` 信号
**问题**: `MushroomHealer.gd` 尝试连接 `damage_blocked` 信号但该信号不存在于 `Unit.gd`

**错误信息**: `ERROR: Nonexistent signal: 'damage_blocked'`

**修复文件**: `src/Scripts/Unit.gd`
- 添加 `signal damage_blocked(damage: float, source: Node)`
- 在 `take_damage()` 中计算并发送被阻挡的伤害
...
#### 修复 3: 奶牛 (cow) 测试配置错误
**问题**: 奶牛测试仅放置支援单位（cow），无防御能力，核心被敌人击杀

**修复文件**: `src/Scripts/Tests/TestSuite.gd`
- 添加 `torch` 单位保护核心
- 增加核心血量: 400→700, max: 500→1000
- 添加明确的敌人配置（单只弱小敌人）
- 修正治疗间隔描述: 5秒→6秒, 1%→1.5%
...
#### 修复 4: 猫头鹰 (owl) 测试配置错误
**问题**: 猫头鹰是支援单位（BuffProviderBehavior），为友军提供暴击率加成，本身不攻击

**修复文件**: `src/Scripts/Tests/TestSuite.gd`
- 添加 `eagle` 单位作为攻击来源
- 保留猫头鹰提供 buff，验证 eagle 产生的命中事件

**状态**: ✅ 已修复

---

#### 修复 5: 鹦鹉 (parrot) 测试配置错误
**问题**: 鹦鹉需要捕捉其他远程单位的子弹才能攻击，单独放置无法攻击

**修复文件**: `src/Scripts/Tests/TestSuite.gd`
- 添加 `squirrel` 远程单位提供子弹来源
- 增加测试时长: 10秒→12秒
- 增加敌人数量: 1→2
...
#### 修复 6: 狐狸 (fox) 测试配置错误
**问题**: 狐狸魅惑触发条件是"被敌人攻击时"，但测试中的 `yak_guardian` 嘲讽敌人，狐狸不会被攻击
...
| 动作类型 | 说明 | 优先级 | 使用场景 |
|---------|------|--------|----------|
| `damage_core` | 扣除核心血量 | P0 | 恶霸犬血量触发机制测试 |
| `heal_core` | 回复核心血量 | P0 | 狮王回蓝、奶牛治疗测试 |
| `record_attack_speed` | 记录攻击速度 | P1 | 恶霸犬攻速变化验证 |
| `record_damage` | 记录伤害数值 | P0 | 猛虎流星雨伤害验证 |
| `record_lifesteal` | 记录吸血量 | P1 | 生命偷取效果验证 |
| `verify_form` | 验证形态 | P2 | 石像鬼石化形态切换 |
| `verify_shield` | 验证护盾值 | P1 | 护盾生成验证 |
| `verify_hp` | 验证血量 | P1 | 单位血量变化验证 |
| `add_soul` | 添加魂魄 | P1 | 猛虎血怒暴击测试 |
| `devour` | 吞噬单位 | P1 | 狼/猛虎吞噬机制测试 |
| `merge` | 合并单位 | P1 | 狼双重继承测试 |
| `attach` | 附身/连接 | P2 | 血祖附身测试 |
| `mimic` | 模仿特效 | P2 | 鹦鹉模仿测试 |
| `apply_buff` | 施加Buff | P0 | Buff效果测试 |
| `place_trap` | 放置陷阱 | P1 | 陷阱类单位测试 |
| `end_wave` | 结束波次 | P1 | 波次结束触发测试 |
| `verify_charm` | 验证魅惑状态 | P1 | 狐狸魅惑机制验证 |
| `verify_clone` | 验证克隆体 | P1 | 羊灵克隆机制验证 |
| `verify_fear` | 验证恐惧效果 | P1 | 狮子恐惧效果验证 |
| `verify_pierce` | 验证穿透次数 | P1 | 蜜蜂穿透验证 |
...
| 验证方法 | 说明 | 使用场景 |
|---------|------|----------|
| `assert_damaged` | 验证受到伤害 | 伤害输出验证 |
| `assert_buff_applied` | 验证Buff已施加 | Buff效果验证 |
| `assert_debuff_applied` | 验证Debuff已施加 | Debuff效果验证 |
| `assert_hp_changed` | 验证血量变化 | 治疗/伤害验证 |
| `assert_mp_changed` | 验证法力变化 | 回蓝机制验证 |
| `assert_target_switched` | 验证目标切换 | 嘲讽/魅惑验证 |
| `assert_clone_spawned` | 验证克隆体生成 | 羊灵克隆验证 |
| `assert_attack_speed_changed` | 验证攻速变化 | 恶霸犬狂暴验证 |
| `assert_charmed` | 验证魅惑状态 | 狐狸魅惑验证 |
| `assert_meteor_spawned` | 验证流星雨生成 | 猛虎技能验证 |
| `assert_shockwave_hit` | 验证冲击波命中 | 狮子冲击波验证 |
| `assert_pierce_count` | 验证穿透次数 | 蜜蜂穿透验证 |
| `assert_fear_applied` | 验证恐惧效果 | 狮子恐惧验证 |
| `assert_soul_gained` | 验证魂魄获得 | 血魂系统验证 |
...
> **重要**: 本节记录那些测试框架**应该发现但没有发现**的Bug，用于改进测试方法和避免重复犯错。
>
> **规则**: 只要出现了用户提到但测试未发现的BUG，都需要记录到此章节，分析原因并提出改进措施。
...
| 检查项 | 状态 | 说明 |
|--------|------|------|
| 测试场景存在 | [是/否] | 是否有对应的测试场景 |
| 触发条件构造 | [是/否] | 是否主动构造了触发条件 |
| 验证逻辑完整 | [是/否] | 是否验证了正确结果 |
| 边界条件覆盖 | [是/否] | 是否测试了边界情况 |
...
#### 遗漏 #1: 攻击流血Debuff敌人没有回血

**发现时间**: 2026-02-21
**发现者**: 用户报告
**相关系统**: 流血吸血系统 (LifestealManager)
**严重程度**: 高

**Bug描述**:
蝙蝠图腾单位的吸血机制失效：当蚊子等蝙蝠单位攻击带有流血Debuff的敌人时，应该按流血层数回复核心生命，但实际没有回血。

**为什么测试没有检测出来**:
1. **测试场景不完整**: `test_bleed_lifesteal`测试用例只配置了蚊子单位，没有配置敌人(`"enemies"`数组缺失)
2. **验证逻辑缺失**: 测试只检查是否能运行不报错，没有验证核心血量是否实际增加
3. **触发条件未构造**: 虽然测试配置了`bat_totem`图腾类型，但没有直接给敌人添加流血debuff，而是依赖图腾每5秒自动施加，但测试时长只有8秒，可能来不及触发
4. **缺少结果断言**: AutomatedTestRunner只记录日志，没有对`core_health`变化做断言验证

**测试缺陷分析**:
| 检查项 | 状态 | 说明 |
|--------|------|------|
| 测试场景存在 | 是 | `test_bleed_lifesteal`存在 |
| 触发条件构造 | 否 | 未配置敌人，未给敌人添加debuff |
| 验证逻辑完整 | 否 | 只验证运行不报错，未验证回血效果 |
| 边界条件覆盖 | 否 | 未测试不同流血层数的回血效果 |

**改进措施**:
1. **修正测试配置**: 在`TestSuite.gd`的`test_bleed_lifesteal`中添加敌人配置，并预设流血debuff
2. **添加验证动作**: 扩展测试框架，添加`verify_core_heal`动作类型，验证核心血量实际增加
3. **主动构造条件**: 使用`setup_actions`直接给敌人添加`bleed` debuff，而不是等待图腾施加
4. **日志断言**: 在测试结束时读取日志，验证是否有`lifesteal_occurred`事件记录

**相关代码位置**:
- `src/Scripts/Tests/TestSuite.gd:70-83` (test_bleed_lifesteal配置)
- `src/Scripts/Tests/AutomatedTestRunner.gd:131-162` (测试执行逻辑，缺少回血验证)
- `src/Scripts/Managers/LifestealManager.gd:13-44` (吸血逻辑)
...
**发现时间**: 2026-02-21
**发现者**: 用户报告
**相关系统**: 狼图腾 - 猛虎单位 (Tiger.gd)
**严重程度**: 高

**Bug描述**:
猛虎单位在使用主动技能(吞噬)时会报错。`on_skill_activated`方法访问`unit.unit_data.get("skillDuration", 5.0)`，但`unit_data`可能不存在或结构不正确。

**为什么测试没有检测出来**:
1. **技能未被触发**: `test_wolf_totem_tiger`测试用例没有配置`scheduled_actions`来触发猛虎的主动技能
2. **被动测试陷阱**: 测试只验证了单位能正常放置和运行，没有测试主动技能流程
3. **代码覆盖率不足**: 虽然测试运行成功，但`on_skill_activated`函数根本没有被调用
4. **缺少技能测试动作**: 测试框架支持`skill`动作，但猛虎的测试用例没有使用

**测试缺陷分析**:
| 检查项 | 状态 | 说明 |
|--------|------|------|
| 测试场景存在 | 是 | `test_wolf_totem_tiger`存在 |
| 触发条件构造 | 否 | 未配置`skill`动作触发技能 |
| 验证逻辑完整 | 否 | 未验证技能释放流程 |
| 边界条件覆盖 | 否 | 未测试不同目标类型的技能效果 |

**改进措施**:
1. **修正测试配置**: 在`test_wolf_totem_tiger`中添加`scheduled_actions`，包含`skill`动作触发吞噬
2. **扩展测试验证**: 在技能触发后验证流星雨是否正确生成
3. **主动技能检查清单**: 为所有带主动技能的单位创建统一的技能测试模板
4. **代码静态分析**: 在测试前检查技能代码中可能存在的空指针访问

**相关代码位置**:
- `src/Scripts/Tests/TestSuite.gd:386-394` (test_wolf_totem_tiger配置，缺少skill动作)
- `src/Scripts/Units/Behaviors/Tiger.gd:6-9` (on_skill_activated，可能访问不存在的属性)
- `src/Scripts/Tests/AutomatedTestRunner.gd:163-178` (skill动作执行逻辑)
...
#### 对于机制类测试
- [ ] 是否配置了触发机制所需的所有实体（敌人/友方单位）
- [ ] 是否主动构造了触发条件（debuff/血量/时间等）
- [ ] 是否有验证机制实际生效的断言（不是只验证不报错）
- [ ] 是否测试了机制的关键数值计算（吸血量/伤害值等）
...
| 测试ID | 问题 | 严重程度 | 备注 |
|--------|------|----------|------|
| test_cow_totem_cow | 治疗不工作 | P0 | 核心血量无变化 |
| test_cow_totem_mushroom_healer | 脚本错误 | P1 | damage_blocked信号不存在 |
| test_cow_totem_mushroom_healer_lv2 | 脚本错误 | P1 | damage_blocked信号不存在 |
| test_bat_totem_mosquito | 吸血不工作 | P0 | 核心血量减少 |
| test_eagle_totem_owl | 无法攻击 | P0 | 0命中事件 |
| test_eagle_totem_parrot | 无法攻击 | P0 | 0命中事件 |
| test_bleed_lifesteal_system | 吸血不工作 | P0 | 核心血量减少 |
| test_charm_system | 魅惑不工作 | P2 | 0命中事件 |

#### 详细报告
完整测试报告: [test_execution_report_2026-02-21.md](test_execution_report_2026-02-21.md)
